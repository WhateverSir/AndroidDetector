    ...
    public void OnRunInferenceButtonClick(View view){
        ...
        runInBackground(
                new Runnable() {
                    ...
                    public void run() {
                        try {
                            double pretime=0,infertime=0,posttime=0;
                            for(int i = 0; i < process_files.length; i++){
                                ...
                                pretime = pretime + e_time-s_time;
                                List<TfliteRunner.Recognition> bboxes = runner.runInference();
                                infertime = infertime + runner.inference_elapsed;
                                posttime = posttime + runner.postprocess_elapsed;
                            ...
                            addLog(String.format("preprocess: %.2fms inference: %.2fms postprocess: %.2fms", pretime/process_files.length,infertime/process_files.length,posttime/process_files.length));
                        } catch (Exception e) {
                            runOnUiThread(
                                    new Runnable() {
                                        @Override
